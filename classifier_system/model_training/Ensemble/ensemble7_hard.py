...
def predict_transformers(model, test_data, softmax: bool = False):
    ...
    return predictions_probabilities.numpy().tolist() if softmax else classes
...
def main(args):
    ...
    svm_test_predictions = svm_clf.predict(X_test_svm)
    train_predictions = [bert_train_predictions, bertweet_train_predictions, svm_train_predictions,
                         ernie_train_predictions, distilbert_train_predictions, distilbert_balanced_train_predictions, roberta_train_predictions]
    ...
    # save_preds = False
    # if save_preds:
    #     numpy.save(file='preds_train_nn5.npy', arr=preds_train_nn)
    #     numpy.save(file='preds_test_nn5.npy', arr=preds_test_nn)
    #
    ...
    # cmd.plot(cmap='Blues')
    # pyplot.savefig('cm_nn_5.png')
    voting_preds = voting_classifier(predictions=test_predictions, num_labels=num_labels, voting='hard')
    ...
    cmd = ConfusionMatrixDisplay(conf_mat, display_labels=['Unrelated', 'Pro-ED', 'Pro-recovery'])
    cmd.plot(cmap='Blues')
    pyplot.savefig('cm_vote_7_hard.png')
