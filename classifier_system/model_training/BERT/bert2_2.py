...
def plot_stats(history, should_show=True):
    ...
    pyplot.xlabel('Epoch')
    pyplot.legend()
    pyplot.savefig('loss_bert2_2.png')
    ...
    pyplot.xlabel('Epoch')
    pyplot.legend()
    pyplot.savefig('acc_bert2_2.png')
    if should_show:
        pyplot.show()
...
def main(args):
    ...
    X_train, X_val, y_train, y_val = create_test_val_dataset(X, y, train_size=0.85)
    ...
    model = TFBertForSequenceClassification.from_pretrained('bert-base-uncased',
                                                            num_labels=3,
                                                            return_dict=True)
    checkpoint_path = "bert2_2_ckpt/cp-{epoch:04d}.ckpt"
