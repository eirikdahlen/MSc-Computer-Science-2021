...
from transformers import BertTokenizerFast, TFBertForSequenceClassification, RobertaTokenizerFast
...
def main(args):
    ...
    num_over = 0
    sum_len = 0
    for encoding in train_encodings[0]:
        sum_len += len(encoding)
        if len(encoding) > 512:
            num_over += 1
    print(f"Avg len: {sum_len/len(train_encodings[0])}")
    print(f"Posts with over 512 tokens: {num_over}, percentage: {num_over/len(train_encodings[0])}")


    # model = TFBertForSequenceClassification.from_pretrained('bert-base-uncased',
    #                                                         num_labels=3,
    #                                                         return_dict=True)
    # checkpoint_path = "bert1_ckpt/cp-{epoch:04d}.ckpt"
