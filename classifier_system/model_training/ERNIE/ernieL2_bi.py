...
def plot_stats(history, should_show=True):
    ...
    pyplot.legend()
    pyplot.savefig('loss_ernieL2_bi.png')
    ...
    pyplot.legend()
    pyplot.savefig('acc_ernieL2_bi.png')
    if should_show:
        pyplot.show()
...
def main(args):
    ...
    y_train, y_val, y_test = to_categorical_labels(y_train, y_val, y_test, binary=True)
    ...
    model = TFAutoModelForSequenceClassification.from_pretrained('nghuyong/ernie-2.0-large-en',
                                                                 num_labels=2,
                                                                 return_dict=True)
    checkpoint_path = "ernieL2_bi_ckpt/cp-{epoch:04d}.ckpt"
